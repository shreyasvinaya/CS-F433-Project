...
root_path = '/Users/shreyasv/Desktop/Comp_Neuro/Relationship_between_latent_dynamics_and_LFP';
data_path = fullfile(root_path,'data');
main_path = fullfile(root_path,'MainAnalysis');
additional_path = fullfile(root_path,'AdditionalAnalysis');
addpath(genpath(main_path));
addpath(genpath(additional_path));
addpath(genpath(data_path));

% Add trial_data repo to path
addpath(genpath(fullfile(root_path,"TrialData")));
addpath(genpath(fullfile(root_path,"AdditionalAnalysis")));
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Figure 2
load(fullfile(data_path,'Chewie_CO_20161021.mat')); % Reduced raw data form Chewie_CO_CS_BL_10212016_001
% t = table2array(kin(:,1));
% trials = [2,5,7,18];
% goCueTime = table2array(trials(:,2));
...
% ch = 34;
% t = table2array(lfp(:,1)); 
% lfp_guide = trial_data.([array '_lfp_guide']);  
...
data_exec = cell2mat(total_cca_exec'); 
data_prep = cell2mat(total_cca_prep'); 
data_rest = cell2mat(total_cca_rest'); 
subplot(1,2,1)
for j = 1:6
    x = data_prep(j,:);
    y = data_exec(j,:);
    for b = 1:9
        if ismember(j,1:3)
            hold on; 
            d(b) = scatter(x(b),y(b),'MarkerEdgeColor',[0,0,0],'MarkerFaceColor',[0,0,0]);
        elseif ismember(j,4:6)
            hold on; 
            a(b) = scatter(x(b),y(b),'square','MarkerEdgeColor',[0,0,0],'LineWidth',2);
        end
    end
end
